...
class PredictView(views.APIView):
    def post(self, request, endpoint_name, format=None):
        ...
        algorithm_name = self.request.query_params.get("name")
        ...
        if algorithm_name is not None:
            algs = algs.filter(name = algorithm_name)
        ...
        # algorithm_object = registry.endpoints[algs[alg_index]]

        prediction = registry.endpoints[algs[alg_index].id].predict(request.data)
        #prediction = 3
